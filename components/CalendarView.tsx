    ...
    for (let day = 1; day <= totalDays; day++) {
      ...
      days.push(
        <div
          ...
          className={`relative h-24 sm:h-28 border-b border-r border-slate-100 dark:border-slate-800 p-2 transition-all cursor-pointer group hover:bg-slate-50 dark:hover:bg-slate-800/50 flex flex-col justify-between ${isSelected ? 'bg-blue-50 dark:bg-blue-900/10' : 'bg-white dark:bg-slate-900'
            }`}
        >
          <div className="flex justify-between items-start">
            <span className={`text-sm font-medium w-7 h-7 flex items-center justify-center rounded-full transition-colors ${isToday
            ...
            {viewMode === 'fiscal' ? (
              <>
                ...
                {dayDeadlines.map((rule, idx) => (
                  <div key={idx} className={`text-[10px] px-1.5 py-0.5 rounded truncate font-medium border flex items-center gap-1 ${rule.category === Category.PAYROLL
              ...
              <>
                {isPayrollDay && (
                  <div className={`text-[10px] px-1.5 py-1 rounded font-bold border flex items-center gap-1 ${payrollStatus === 'PROCESADO'
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {[
                    ...
                    <motion.div
                      key={month}
                      whileHover={{ y: -5 }}
                      className={`p-6 rounded-[2rem] border-2 transition-all ${annualBudgetForm.amounts[idx] > 0
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div className="flex bg-slate-200 dark:bg-slate-800 p-1 rounded-xl">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${viewMode === 'fiscal'
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${viewMode === 'payroll'
                    ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm'
                    : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'
                  }`}
              >
                    ...
                    {isPayrollDay && (
                      <div className="p-4 rounded-xl border border-blue-100 dark:border-blue-900/30 bg-blue-50 dark:bg-blue-900/10">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span className={`text-[10px] px-2 py-0.5 rounded-full ${monthPayroll.length > 0 && monthPayroll.every(e => e.status === 'PROCESADO')
          ...
          {dayEvents.deadlines.length > 0 && (
            <div className="space-y-3">
              ...
              {dayEvents.deadlines.map((rule, idx) => (
                <div key={idx} className={`p-4 rounded-xl border ${rule.category === Category.PAYROLL
                  ...
                  <div className="flex justify-between items-start mb-1">
                    <span className={`font-bold text-sm ${rule.category === Category.PAYROLL ? 'text-orange-700 dark:text-orange-300' : 'text-purple-700 dark:text-purple-300'
                      }`}>{rule.title}</span>
                    <span className={`text-[10px] px-2 py-0.5 rounded-full ${rule.category === Category.PAYROLL
                  ...
                  <div className={`mt-2 flex items-center gap-1 text-[10px] font-medium ${rule.category === Category.PAYROLL ? 'text-orange-600 dark:text-orange-400' : 'text-purple-600 dark:text-purple-400'
                    }`}>
            ...
            <div className="space-y-3">
              ...
              {dayEvents.realPayments.map(payment => (
                <div key={payment.id} className="p-4 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-colors group">
                  <div className="flex justify-between items-start mb-2">
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-md ${payment.status === PaymentStatus.OVERDUE ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
