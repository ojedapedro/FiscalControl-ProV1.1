...
export const UTILITY_TAX_CONFIG: Record<string, { label: string; deadlineDay: number; items: { code: string; name: string; amount?: number; isVariable?: boolean }[] }> = {
  'SERVICIOS': {
    label: '10 SERVICIOS PUBLICOS',
    deadlineDay: 30,
    items: [
      { code: '10.1', name: 'ELECTRICIDAD (CORPOELEC)', isVariable: true },
      { code: '10.2', name: 'AGUA (HIDROCAPITAL/HIDROCENTRO)', isVariable: true },
      { code: '10.3', name: 'TELEFONO E INTERNET (CANTV)', isVariable: true },
      { code: '10.4', name: 'ASEO URBANO (FOSPUCA/OTROS)', isVariable: true },
      { code: '10.5', name: 'GAS', isVariable: true },
      { code: '10.6', name: 'CONDOMINIO', isVariable: true },
    ]
  }
};

export const INVENTORY_TAX_CONFIG: Record<string, { label: string; deadlineDay: number; items: { code: string; name: string; amount?: number; isVariable?: boolean }[] }> = {
  'INVENTARIO': {
    label: '12 INVENTARIO',
    deadlineDay: 30,
    items: [
      { code: '12.1', name: 'COMPRA DE MERCANCIA NACIONAL', isVariable: true },
      { code: '12.2', name: 'COMPRA DE MERCANCIA IMPORTADA', isVariable: true },
      { code: '12.3', name: 'SUMINISTROS', isVariable: true },
    ]
  }
};

export const OTHER_TAX_CONFIG: Record<string, { label: string; deadlineDay: number; items: { code: string; name: string; amount?: number; isVariable?: boolean }[] }> = {
  'OTROS': {
    label: '13 OTROS PAGOS',
    deadlineDay: 30,
    items: [
      { code: '13.1', name: 'MANTENIMIENTO GENERAL', isVariable: true },
      { code: '13.2', name: 'REPARACIONES MENORES', isVariable: true },
      { code: '13.3', name: 'GASTOS VARIOS', isVariable: true },
    ]
  }
};
