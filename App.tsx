  ...
  const renderContent = () => {
    ...
    switch (currentView) {
      ...
      case 'calendar':
        return <CalendarView />;
      case 'settings':
        return <div className="p-10 text-center text-slate-400">Módulo de Configuración (Próximamente)</div>;
      default:
  ...
  return (
    <div className="flex bg-slate-50 min-h-screen font-sans text-slate-900">
      ...
      {notification && currentView !== 'notifications' && (
        <div className="fixed bottom-6 right-6 bg-white border-l-4 border-yellow-500 shadow-2xl p-4 rounded-lg z-50 animate-in slide-in-from-right duration-500 max-w-sm flex flex-col gap-3">
          <div className="flex gap-4 items-start">
            <div className="text-yellow-600 mt-1">
               <BellRing size={20} />
            </div>
            <div>
              <h4 className="font-bold text-slate-800 text-sm">Recordatorio Fiscal</h4>
              <p className="text-sm text-slate-600 mt-1 leading-relaxed">{notification}</p>
            </div>
            <button 
          ...
          {pushPermission === 'default' && (
            <button 
                ...
                className="w-full flex items-center justify-center gap-2 bg-slate-900 text-white text-xs font-bold py-2 rounded-md hover:bg-slate-800 transition-colors"
            >
                <Bell size={14} />
